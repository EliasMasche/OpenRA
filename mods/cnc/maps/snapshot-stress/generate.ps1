# Regenerates this map: one of every placeable cnc actor, alternating sides, a self-playing bot,
# ore beside each refinery, and a water bay with the naval actors.
#
#   pwsh mods/cnc/maps/snapshot-stress/generate.ps1
#   bin/OpenRA.Utility.exe cnc --map refresh snapshot-stress   # regenerates the preview and map.yaml
#
# The terrain is flat TEMPERAT clear ground (template 0) with a water bay (template 1) and tiberium
# (resource index 1) with a little blue tiberium (resource index 2). This is the cnc copy of
...
# --- per-mod parameters ------------------------------------------------------------------------
$mod = 'cnc'
$tilesetFile = 'temperat'   # mods/<mod>/tilesets/<file>.yaml
$tilesetName = 'TEMPERAT'   # what map.yaml declares
$mapW = 140
$mapH = 140
...
$groundTile = 0; $groundTerrain = 'Clear'; $groundVariants = 16
$waterTile = 1; $waterTerrain = 'Water'
# The resource layer's AllowedTerrainTypes decides which tile ore may sit on. On cnc that is Clear,
# so the ore cells reuse the plain ground tile.
$oreGroundTile = $groundTile
...
# separately. Files not listed here (ai, player, world, palettes, defaults, husks, map-generators)
# hold no placeable actors.
$plan = [ordered]@{
...
$basePlan = @(
	@('fact', 0, 0), @('nuke', 8, 0), @('nuke', 12, 0), @('proc', 4, 4), @('pyle', 12, 4),
	@('weap', 16, 4), @('fix', 20, 4), @('afld', 4, 10), @('hpad', 10, 10), @('gun', 20, 10),
	@('sam', 18, 12), @('gtwr', 22, 12), @('harv', 16, 22), @('harv', 18, 24), @('harv', 20, 22),
	@('mcv', 6, 18)
)
...
$combat = @('e1', 'e2', 'e3', 'e4', 'e5', 'e6', 'apc', 'arty', 'ftnk', 'bggy', 'bike', 'jeep',
	'ltnk', 'mtnk', 'htnk', 'msam', 'mlrs', 'harv')
...
Assert-Tile $tileset $oreGroundTile $groundTerrain 1 'ore ground'
...
foreach ($b in @(@{ X = SX 6; Y = SY 94 }, @{ X = SX 104; Y = SY 94 })) {
	...
	for ($x = $ox; $x -lt [Math]::Min($ox + $oreWidth, $W - 1); $x++) {
		for ($y = $oreTop; $y -le $oreBottom; $y++) {
			...
			if ($oreGroundTile -ne $groundTile) { Set-Tile $x $y $oreGroundTile (($x + $y) % 4) }
			Set-Ore $x $y $oreResource $oreDensity
		}
	}

	# A few gem cells, so the second resource path is exercised too.
